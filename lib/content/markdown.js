...
function markdownToHtml(markdown) {
  ...
  lines.forEach((line) => {
    ...
    if (inRawHtmlTag) {
      html.push(line);
      const closePattern = new RegExp(`</${inRawHtmlTag}>`, 'i');
    ...
    const hasPreOpen = /<pre\b[^>]*>/i.test(trimmedLine);
    const hasPreClose = /<\/pre>/i.test(trimmedLine);
    const hasCodeOpen = /<code\b[^>]*>/i.test(trimmedLine);
    const hasCodeClose = /<\/code>/i.test(trimmedLine);
